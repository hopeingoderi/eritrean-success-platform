...
(() => {
  ...
  const API_BASE = (() => {
    ...
    if (host === "riseeritrea.com" || host === "www.riseeritrea.com") return "https://api.riseeritrea.com/api";
    if (host === "api.riseeritrea.com") return "https://api.riseeritrea.com/api";
  ...
  async function api(path, { method = "GET", body } = {}) {
    ...
    const url = `${API_BASE}${path}${join}lang=${encodeURIComponent(getLang())}`;
  ...
  function courseTitle(c) {
    // backend/courses.js returns { title, intro } (already localized by lang middleware)
    return c?.title || c?.id || "";
  }
  ...
  window.go = function (page) {
    if (page.startsWith("#/")) setHash(page);
    else if (page === "login") setHash("#/login");
  ...
  async function render() {
    // always refresh lang from storage (in case another tab changed it)
    state.lang = getLang();
      ...
      <div class="card">
        <div class="row">
          ...
          <div class="row" style="gap:8px; justify-content:flex-end;">
            <button class="btn" id="langEn" ${state.lang === "en" ? "disabled" : ""}>English</button>
            <button class="btn" id="langTi" ${state.lang === "ti" ? "disabled" : ""}>ትግርኛ</button>
          </div>
        </div>
      </div>
  ...
  async function renderCourse(courseId) {
    ...
    const listHtml = lessons
      .slice()
      .sort((a, b) => (a.lessonIndex ?? a.lesson_index ?? 0) - (b.lessonIndex ?? b.lesson_index ?? 0))
      .map(l => {
        const idx = (l.lessonIndex ?? l.lesson_index ?? 0);
  ...
  async function renderLesson(courseId, lessonIndexStr) {
    ...
    const lessons = state.lessonsByCourse[courseId] || [];
    const lesson = lessons.find(x => (x.lessonIndex ?? x.lesson_index) === lessonIndex);
    ...
    const prevExists = lessons.some(x => (x.lessonIndex ?? x.lesson_index) === lessonIndex - 1);
    const nextExists = lessons.some(x => (x.lessonIndex ?? x.lesson_index) === lessonIndex + 1);

    // lesson fields already localized by backend lessons route, usually: title, learn, task
    const learnText = lesson.learnText || lesson.learn || "";
    const taskText = lesson.task || "";
    ...
    document.getElementById("lessonCard").innerHTML = `
      ...
      <div class="p">${escapeHtml(learnText)}</div>

      <div style="height:10px"></div>
      <div class="h2" style="font-size:16px;">Task</div>
      <div class="p">${escapeHtml(taskText)}</div>
  ...
  async function renderCert(courseId) {
    ...
    if (status.issued) {
      document.getElementById("certCard").innerHTML = `
        ...
        <div style="height:12px"></div>
        <a class="btn primary" href="${API_BASE}/certificates/${courseId}/pdf?lang=${encodeURIComponent(getLang())}"
    ...
    document.getElementById("claimBtn").onclick = async () => {
      ...
      try {
        ...
        window.open(`${API_BASE}/certificates/${courseId}/pdf?lang=${encodeURIComponent(getLang())}`, "_blank");
